...
import commandsModule from '../../../../extensions/cornerstone/src/commandsModule';
...
const module = commandsModule(commandsManager);
...
function setLevelPreset(window, level) {
  const state = store.getState();
  const viewports = state.viewports;
  module.actions.setWindowLevel({
    viewports,
    window,
    level,
  });
...
function _getExpandableButtonComponent(button, activeButtons) {
  ...
  const childButtons = button.buttons.map(childButton => {
    let onClick;
    if (childButton.presets) {
      onClick = () => setLevelPreset(childButton.window, childButton.level);
    } else {
