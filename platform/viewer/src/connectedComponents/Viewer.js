...
const _checkForSeriesInconsistencesWarnings = async function(
  ...
  if (displaySet.Modality !== 'SEG') {
    if (
      ...
      displaySet.reconstructionIssues.forEach(warning => {
        switch (warning) {
          case ReconstructionIssues.DATASET_4D:
            inconsistencyWarnings.push('O conjunto de dados é 4D.');
            break;
          case ReconstructionIssues.VARYING_IMAGESDIMENSIONS:
            inconsistencyWarnings.push(
              'Os frames do conjunto de dados têm dimensões diferentes (linhas, colunas).'
            );
            break;
          case ReconstructionIssues.VARYING_IMAGESCOMPONENTS:
            inconsistencyWarnings.push(
              'Os frames do conjunto de dados têm componentes diferentes (amostra por pixel).'
            );
            break;
          case ReconstructionIssues.VARYING_IMAGESORIENTATION:
            inconsistencyWarnings.push(
              'Os frames do conjunto de dados têm orientação diferente.'
            );
            break;
          case ReconstructionIssues.IRREGULAR_SPACING:
            inconsistencyWarnings.push(
              'Os frames do conjunto de dados têm espaçamento de pixel diferente.'
            );
            break;
          case ReconstructionIssues.MULTIFFRAMES:
            inconsistencyWarnings.push('O conjunto de dados é um multiframes.');
            break;
          default:
            break;
        }
      ...
      inconsistencyWarnings.push(
        'Os conjuntos de dados não são um volume 3D reconstruível. O modo MPR não está disponível.'
      );
    }
    ...
    if (
      ...
      inconsistencyWarnings.push(
        'Os conjuntos de dados estão sem frames: ' +
          displaySet.missingFrames +
          '.'
      );
    }
    ...
    for (
      ...
      if (!SourceImageSequence) {
        if (inconsistencyWarnings.length === 0) {
          const warningMessage =
            'A segmentação ' +
            'tem frames fora do plano em relação às imagens de origem.';
      ...
      if (
        ...
        const warningMessage =
          'A segmentação ' +
          'tem molduras com geometria diferente ' +
          'dimensões (Linhas e Colunas) em relação às imagens de origem.';
        inconsistencyWarnings.push(warningMessage);
        break;
      }
    }
    ...
    if (inconsistencyWarnings.length !== 0) {
      const warningMessage =
        'O formato de segmentação ainda não é compatível. ' +
        'Os dados de segmentação (segmentos) não puderam ser carregados.';
      inconsistencyWarnings.push(warningMessage);
    }
  }
