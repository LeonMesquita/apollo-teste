...
class MetadataProvider {
  ...
  _getCornerstoneWADOImageLoaderTag(wadoImageLoaderTag, instance) {
    ...
    switch (wadoImageLoaderTag) {
      ...
      case WADO_IMAGE_LOADER_TAGS.IMAGE_PIXEL_MODULE:
        ...
        //   my_photo = "RGB"
        // } else {
        //   my_photo = instance.photometricInterpretation
        // }
        ...
        metadata = {
          ...
          photometricInterpretation:
            instance.PlanarConfiguration !== undefined
              ? 'RGB'
      ...
      case WADO_IMAGE_LOADER_TAGS.PATIENT_MODULE:
        const { PatientName, PatientBirthDate, PatientAge } = instance;
        ...
        if (PatientBirthDate) {
          patientBirthDate = this._formatDate(PatientBirthDate, 'DD/MM/YYYY');
        }

        let patientAge;
        if (PatientAge) {
          patientAge = PatientAge.replace('Y', ' Anos');
          if (patientAge.charAt(0) === '0') {
            patientAge = patientAge.slice(1);
          }
        } else if (PatientBirthDate) {
          const studyDate = this._formatDate(instance.StudyDate, 'YYYY-MM-DD');
          const birthDate = this._formatDate(PatientBirthDate, 'YYYY-MM-DD');
          patientAge = this._calculateAge(birthDate, studyDate) + ' Anos';
        }
  ...
  _calculateAge(birthDate, studyDate) {
    const birth = new Date(birthDate);
    const study = new Date(studyDate);

    const difference = study.getTime() - birth.getTime();
    const ageMilisseconds = new Date(difference);
    return Math.abs(ageMilisseconds.getUTCFullYear() - 1970);
  }
