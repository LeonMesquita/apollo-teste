...
export default function getPixelSpacing(instance) {
  ...
  if (isProjection === false && !ImagerPixelSpacing && PixelSpacing) {
    // If only Pixel Spacing is present, and this is not a projection radiograph,
    // we can stop here
    // console.log('isProjection === false && !ImagerPixelSpacing');
    return {
      PixelSpacing,
    ...
    // console.log(
    //   'PixelSpacing && ImagerPixelSpacing && PixelSpacing === ImagerPixelSpacing'
    // );
    return {
      PixelSpacing,
      type: TYPES.DETECTOR,
    ...
    // console.log('!PixelSpacing && ImagerPixelSpacing');
    ...
    // console.log(SequenceOfUltrasoundRegions);
    var valid_measurements = undefined;
    if (SequenceOfUltrasoundRegions.length > 0) {
      for (var i = 0; i < SequenceOfUltrasoundRegions.length; i++) {
        const { PhysicalDeltaX, PhysicalDeltaY } = SequenceOfUltrasoundRegions[
          i
        ];
        if (PhysicalDeltaX && PhysicalDeltaY) {
          valid_measurements = SequenceOfUltrasoundRegions[i];
          break;
        }
      }
    } else {
      valid_measurements = SequenceOfUltrasoundRegions;
    }

    const { PhysicalDeltaX, PhysicalDeltaY } = valid_measurements;
    const USPixelSpacing = [PhysicalDeltaX * 10, PhysicalDeltaY * 10];
    // console.log(USPixelSpacing);
    // console.log(
    //   "SequenceOfUltrasoundRegions && typeof SequenceOfUltrasoundRegions === 'object'"
    // );
