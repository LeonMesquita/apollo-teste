...
const commandsModule = ({ commandsManager, servicesManager }) => {
  ...
  const actions = {
    ...
    render: async ({ viewports }) => {
      ...
      //Set the data to localSorage so it can be used in Render3D.js
      localStorage.setItem(
        '@viewport-series/apollo',
        JSON.stringify(displaySet)
      );
      ...
      const overlay = document.getElementById('select_CT_xfer_fn');
