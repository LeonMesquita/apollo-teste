...
const OHIFCornerstoneViewportOverlay = ({
  scale,
  windowWidth,
  windowCenter,
  imageId,
  ...
  useEffect(() => {
    const findPreset = Object.entries(preferences.windowLevelData).find(
      ([id, data]) =>
        Number(data.window) === Number(windowWidth) &&
        Number(data.level) === Number(windowCenter)
    );
    if (findPreset) {
      setActivePreset(findPreset[1].description);
    } else {
  ...
  const inconsistencyWarningsOn =
    inconsistencyWarnings && inconsistencyWarnings.length !== 0 ? true : false;
  ...
  const normal = (
    <React.Fragment>
      <div className="top-left overlay-element">
        ...
        <div>{patientId}</div>
        {patientBirthDate ? (
          <div>Nascimento: {formatPN(patientBirthDate)}</div>
        ) : null}
        {patientAge ? <div>Idade: {formatPN(patientAge)}</div> : null}
      </div>
